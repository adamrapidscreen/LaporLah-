...
## Dependencies
- Stories: E7-S2 (castVote server action), E1-S2 (DB functions: award_points, check_and_award_badges, notify_followers)
...
1. `src/lib/actions/votes.ts`: Add `checkResolution(reportId: string)` function with the following logic:
   - Fetch the report (status, resolved_at, creator_id) and all confirmations for this report
   - Count confirmed vs not_yet votes
   - **If confirmed ≥ 3**: UPDATE report status to 'closed', award 25 pts to report creator, award 15 pts to resolver (user who set status to 'resolved'), call `check_and_award_badges` for both users, call `notify_followers` with type 'status_change'
...
async function checkResolution(reportId: string) {
  ...
  const { data: report } = await supabase
    .from('reports')
    .select('id, status, resolved_at, creator_id, resolved_by')
    .eq('id', reportId)
    .single();
...
async function closeReport(supabase: any, report: any) {
  ...
  await supabase.rpc('award_points', {
    p_user_id: report.creator_id,
    p_action: 'report_closed',
    p_points: 25,
    p_report_id: report.id,
  });

  // Award 15 pts to resolver
  ...
  await supabase.rpc('check_and_award_badges', { p_user_id: report.creator_id });
...
if (report.status === 'resolved' && report.resolved_at) {
  ...
  if (hoursElapsed >= 72) {
    // Import checkResolution dynamically or call directly
...
- `resolved_by` column tracks who proposed resolution (needed for 15pt award)
...
- [ ] Report creator receives 25 points when report is closed
- [ ] Resolver receives 15 points when resolution is confirmed
