...
4. `.env.local.example`: Document required environment variables (`NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY`, `SUPABASE_SERVICE_ROLE_KEY`, `NEXT_PUBLIC_APP_URL`, `ADMIN_USER_ID`)
...
- [ ] `reports` table includes `user_id`, `comments_locked`, `resolved_at`, and `resolved_by` columns
- [ ] Indexes exist on frequently queried columns (reports.status, reports.category, reports.user_id, comments.report_id, follows.report_id)
