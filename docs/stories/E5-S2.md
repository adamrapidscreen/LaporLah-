...
## Description
Create a Client Component comment form with a text input and send button. The form uses optimistic updates on submit and wires to the `addComment` server action. Handles locked comments and validates input length.
...
## Tasks
1. `src/components/comments/comment-form.tsx`: Create Client Component with text input (Textarea or Input) and send button. Implement optimistic update using `useOptimistic`. Validate content (min 1 char, max 1000 chars). Show error message if `comments_locked` is true. Clear input on successful submit. Wire to `addComment` server action.
...
```tsx
// For optimistic updates, the parent component wraps CommentList
// with useOptimistic to show the new comment immediately

// In a wrapper client component or using the form action pattern:
...
- `comments_locked`: boolean flag from report — if true, show locked message instead of form
...
- [ ] Optimistic update shows new comment immediately
